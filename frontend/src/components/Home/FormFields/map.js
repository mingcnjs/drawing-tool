                  ...
                  for (let ipoints = 0; ipoints < drawControl.poly._latlngs[0].length; ipoints++)
                      {
                       ...
                       this.und.spos.push([drawControl.poly._latlngs[0][ipoints]["lat"],drawControl.poly._latlngs[0][ipoints]["lng"]]);
                      }    
                  this.shapeGeoJSON.features[ift].geometry.coordinates[0].push(this.shapeGeoJSON.features[ift].geometry.coordinates[0][0]);
                  polygon = turf.polygon(this.shapeGeoJSON.features[ift].geometry.coordinates);
                  rewind = turf.rewind(polygon);
                  this.shapeGeoJSON.features[ift].geometry.coordinates[0] = rewind.geometry.coordinates[0];
                  ...
                  for (let ipoints = 0; ipoints < drawControl.layer._latlngs[0].length; ipoints++)
                      {
                       ...
                       this.und.spos.push([drawControl.layer._latlngs[0][ipoints]["lat"],drawControl.layer._latlngs[0][ipoints]["lng"]]);
                      }  
                  this.undo.push(this.und);    
                  if (this.undo.length-2 > -1)
                     {     
              ...
              if (this.shapeGeoJSON.features[ift].id.toUpperCase() === id.toUpperCase() &&  ptype === "circle")
                 {
                  ...
                  this.und.spos = [drawControl.layer._latlng.lat,drawControl.layer._latlng.lng];  
                  this.und.obj = drawControl.layer;
                  let polygon  = "";
                  this.und.radius = 0;
                  this.und.include = false;  
       ...
       { 
        ...
        for (let ipoints = 0; ipoints < drawControl.layer._latlngs[0].length; ipoints++)
            { 
             ...
             this.und.spos.push([drawControl.layer._latlngs[0][ipoints]["lat"],parseFloat(drawControl.layer._latlngs[0][ipoints]["lng"])]);
            } 
       ...
       if (this.featuregroup.leafletElement._layers[idlayers].id === this.state.selectedid)
          {
           ...
           this.und.stype = this.featuregroup.leafletElement._layers[idlayers].id.split("#")[1];
           this.und.spos = []; 
           this.und.include = false;
           this.und.obj = this.featuregroup.leafletElement._layers[idlayers];
           if (this.und.stype === "rectangle" || this.und.stype === "polygon")
              {
               for (let ipoints = 0; ipoints < this.featuregroup.leafletElement._layers[idlayers]._latlngs[0].length; ipoints++)
                   { 
                    this.und.spos.push([parseFloat(this.featuregroup.leafletElement._layers[idlayers]._latlngs[0][ipoints]["lat"]),parseFloat(this.featuregroup.leafletElement._layers[idlayers]._latlngs[0][ipoints]["lng"])]);
                   }    
              }
           else
              {
               this.und.spos = [parseFloat(this.featuregroup.leafletElement._layers[idlayers]._latlng["lat"]),parseFloat(this.featuregroup.leafletElement._layers[idlayers]._latlng["lng"])]
               this.und.radius = this.featuregroup.leafletElement._layers[idlayers]._mRadius;  
              }       
                        ...
                        for (let ipointsa = 0; ipointsa < this.shapeGeoJSON.features[ifeat].geometry.coordinates[0].length; ipointsa++)
                            {
                             ...
                             this.und.spos.push([this.shapeGeoJSON.features[ifeat].geometry.coordinates[0][ipointsa][1],this.shapeGeoJSON.features[ifeat].geometry.coordinates[0][ipointsa][0]])                        
                            }
                        ...
                        for (let ipointsb = 0; ipointsb < this.shapeGeoJSON.features[ifeat].geometry.coordinates[0].length; ipointsb++)
                            {
                             ...
                             this.und.spos.push([this.shapeGeoJSON.features[ifeat].geometry.coordinates[0][ipointsb][1],this.shapeGeoJSON.features[ifeat].geometry.coordinates[0][ipointsb][0]])                        
                            } 
                    ...
                    if (this.shapeGeoJSON.features[ifeat].others.basetype === "Circle")
                       {
                        ...
                        poly = circle;
                        this.und.spos = [this.shapeGeoJSON.features[ifeat].others.center[1],this.shapeGeoJSON.features[ifeat].others.center[0]];  
                        this.und.id = poly.id;
                        this.und.radius = radius;
                        this.und.stype = this.shapeGeoJSON.features[ifeat].others.basetype;
                        this.und.obj = poly;
                       } 
      ...
      <div style={styles.mapContainer}>
        <div className="draw-toolbar" style={styles.dropdown}>
          ...
          <Button onClick= {() => {
               ...
               let undos = this.undo.sort((a,b)=>{return a.order-b.order});
               let poly = null;
               let fpolygon = null;
               let frewind = null; 
               if (undos.length-1 > 0 && undos[undos.length-1].type === "edit")
                  { 
                   for (let icheck = 0; icheck < this.undo.length; icheck++)
                       {
                        if (this.undo[icheck].id === undos[undos.length-1].id && this.undo[icheck].order < undos[undos.length-1].order && this.undo[icheck].type === "add")
                           {
               ...
               if (undos.length === 0  || (this.undo.length === 1 && this.undo[0].type === "edit" && !this.undo[0].include))
                  {
                   for (let iund = 0; iund < this.unddo.length; iund++)
                       {
                        for (let iddlayers in this.featuregroup.leafletElement._layers)
                            {
                             if (this.featuregroup.leafletElement._layers[iddlayers].id === this.unddo[iund].id)
                                {
                                 this.featuregroup.leafletElement.removeLayer(this.featuregroup.leafletElement._layers[iddlayers]);
                                }
                            }   
                        if (this.unddo[iund].btype === "add")
                           {
                            ...
                            for (let ifts = 0; ifts < this.shapeGeoJSON.features.length; ifts++)
                                {
                                 if (this.shapeGeoJSON.features[ifts].id === this.unddo[iund].id)
                                    {
                                     continue;
                                    }
                                 n3shapeGeoJSON.features.push(this.shapeGeoJSON.features[ifts]);          
                                }
                        ...
                        if (this.unddo[iund].stype.toLowerCase() === "polygon")
                           {  
                            ...
                            for (let iftt = 0; iftt < this.shapeGeoJSON.features.length; iftt++)
                                {
                                 if (this.shapeGeoJSON.features[iftt].id.toUpperCase() === this.unddo[iund].id.toUpperCase())
                                    {
                                     this.shapeGeoJSON.features[iftt].geometry.coordinates = [];
                                     this.shapeGeoJSON.features[iftt].geometry.coordinates.push([]);
                                     for (let ipoints = 0; ipoints < this.unddo[iund].spos.length; ipoints++)
                                         {
                                          this.shapeGeoJSON.features[iftt].geometry.coordinates[0].push([this.unddo[iund].spos[ipoints][1],this.unddo[iund].spos[ipoints][0]]);
                                         }  
                                     this.shapeGeoJSON.features[iftt].geometry.coordinates[0].push(this.shapeGeoJSON.features[iftt].geometry.coordinates[0][0]);
                                     fpolygon = turf.polygon(this.shapeGeoJSON.features[iftt].geometry.coordinates);
                                     frewind = turf.rewind(fpolygon);
                                     this.shapeGeoJSON.features[iftt].geometry.coordinates[0] = frewind.geometry.coordinates[0];
                                    }
                                 }   
                         ...
                         if (this.unddo[iund].stype.toLowerCase() === "rectangle")
                          ...
                          if (this.unddo[iund].stype.toLowerCase() === "circle")
                             {   
                              let pos = L.latLng(this.unddo[iund].spos[0],this.unddo[iund].spos[1]);
                                   ...
                                   if (this.shapeGeoJSON.features[iftt].id.toUpperCase() === this.unddo[iund].id.toUpperCase())
                                      {
                                       ...
                                       let center = [this.unddo[iund].spos[1],this.unddo[iund].spos[0]];  
                                       if (this.unddo[iund].radius > 0)
                                          {
                                           fpolygon = turf.circle(center, this.unddo[iund].radius / 1000, options);
                                           this.shapeGeoJSON.features[iftt].geometry.coordinates[0] = fpolygon.geometry.coordinates[0];
                                          }  
                                       this.shapeGeoJSON.features[iftt].others = {basetype:"Circle",center:center};
                                       this.shapeGeoJSON.features[iftt].properties.radius = this.unddo[iund].radius;
                                      }
                                   }   
                              let stringshapeGeoJSON = this.shapeGeoJSON;
                              this.props.geojsontostate(stringshapeGeoJSON);     
                          ...
                          if (this.drawobject._container.querySelector('[title="Save changes"]'))
                             {     
                              this.drawobject._container.querySelector('[title="Save changes"]').setAttribute("id","savechanges");
                              this.drawobject._container.querySelector("#savechanges").click();
                             } 
                          this.drawobject._container.querySelector("#editpolygon").click();
                      }
                   this.undo = []; 
               ...
               if (lstcnt === 0 && undos.length === 1 && undos[0].type === "edit")
                  {
                   lstcnt = 1;
                   lstrev = undos[0];   
                  }    
               ...
               if (lastrevert.type === "add")
                  {
                   for (let iddlayers in this.featuregroup.leafletElement._layers)
                       {
                        if (this.featuregroup.leafletElement._layers[iddlayers].id === lastrevert.id)
                           {
                            this.featuregroup.leafletElement.removeLayer(this.featuregroup.leafletElement._layers[iddlayers]);
                           }
                       }  
                   ...
                   for (let ifts = 0; ifts < this.shapeGeoJSON.features.length; ifts++)
                       {
                        ...
                        n2shapeGeoJSON.features.push(this.shapeGeoJSON.features[ifts]);          
                       } 
               ...
               if (lastrevert.type === "edit")
                  {
                   ...
                   if (lastrevert.stype.toLowerCase() === "polygon")
                      {   
                       ...
                       for (let iftt = 0; iftt < this.shapeGeoJSON.features.length; iftt++)
                           {
                            if (this.shapeGeoJSON.features[iftt].id.toUpperCase() === lastrevert.id.toUpperCase())
                               {
                                this.shapeGeoJSON.features[iftt].geometry.coordinates = [];
                                this.shapeGeoJSON.features[iftt].geometry.coordinates.push([]);
                                for (let ipoints = 0; ipoints < lastrevert.spos.length; ipoints++)
                                    {
                                     this.shapeGeoJSON.features[iftt].geometry.coordinates[0].push([lastrevert.spos[ipoints][1],lastrevert.spos[ipoints][0]]);
                                    }  
                                this.shapeGeoJSON.features[iftt].geometry.coordinates[0].push(this.shapeGeoJSON.features[iftt].geometry.coordinates[0][0]);
                                fpolygon = turf.polygon(this.shapeGeoJSON.features[iftt].geometry.coordinates);
                                frewind = turf.rewind(fpolygon);
                   ...
                   if (lastrevert.stype.toLowerCase() === "circle")
                      {   
                       poly = L.circle(L.latLng(lastrevert.spos[0],lastrevert.spos[1]),{radius:lastrevert.radius});
                       poly.id = lastrevert.id;                         
                       this.featuregroup.leafletElement.addLayer(poly);
                       poly._path.setAttribute("id",lastrevert.id);
                       for (let iftt = 0; iftt < this.shapeGeoJSON.features.length; iftt++)
                            ...
                            if (this.shapeGeoJSON.features[iftt].id.toUpperCase() === lastrevert.id.toUpperCase())
                               {
                                ...
                                if (lastrevert.radius > 0)
                                   {
                                    fpolygon = turf.circle(center, lastrevert.radius / 1000, options);
                                    this.shapeGeoJSON.features[iftt].geometry.coordinates[0] = fpolygon.geometry.coordinates[0];
                                   }  
                                this.shapeGeoJSON.features[iftt].others = {basetype:"Circle",center:center};
                                this.shapeGeoJSON.features[iftt].properties.radius = lastrevert.radius;
                               }
                           }   
                       let stringshapeGeoJSON = this.shapeGeoJSON;
                       this.props.geojsontostate(stringshapeGeoJSON);  
               ...
               if (lastrevert.type === "remove")
                  {
                   ...
                   if (lastrevert.stype.toLowerCase() === "polygon")
                      {  
                       ...
                       for (let ipoints = 0; ipoints < lastrevert.spos.length; ipoints++)
                           { 
                            feature.geometry.coordinates[0].push([lastrevert.spos[ipoints][1],parseFloat(lastrevert.spos[ipoints][0])]);
                           } 
                       feature.geometry.coordinates[0].push(feature.geometry.coordinates[0][0]);
                       fpolygon = turf.polygon(feature.geometry.coordinates);
                       frewind = turf.rewind(fpolygon);
                       feature.geometry.coordinates[0] = frewind.geometry.coordinates[0];
                       this.shapeGeoJSON.features.push(feature);
                      }   
                   if (lastrevert.stype.toLowerCase() === "rectangle")
                      {  
                       ...
                       if (radius > 0)
                          {
                           ...
                           feature.geometry.coordinates[0] = fpolygon.geometry.coordinates[0];
                          }  
                       feature.others = {basetype:"Circle",center:center};
                       feature.properties.radius = radius;
                       this.shapeGeoJSON.features.push(feature);
